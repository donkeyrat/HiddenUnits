...
public class FindTaggedUnitTargeting : JobComponentSystem
{
	private struct UnitFilter
	{
		...
		[ReadOnly] public ComponentDataArray<FindTaggedUnitTarget> TargetFurthestEnemyTag;
	...
	private struct TargetFilter
	{
		...
		public ComponentDataArray<TaggedUnitTag> TaggedUnit;
		...
		public void Execute()
		{
			for (var i = 0; i < UnitEntities.Length; i++)
			{
				...
				for (var j = 0; j < TargetEntities.Length; j++)
				{
					...
					var currentTargetValue = (0f - distanceToTargetMagnitude) / TargetPriorities[j].Value - value2 * range.AttackRange * 0.05f;
	...
	protected override JobHandle OnUpdate(JobHandle inputDeps)
	{
		...
		var componentDataArray2 = componentGroup.GetComponentDataArray<PredictedPosition>();
		...
		var componentDataArray7 = componentGroup.GetComponentDataArray<PredictedPosition>();
